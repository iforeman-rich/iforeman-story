...
# -*- coding: utf-8 -*-
"""
server.py — Editor lokal content.json untuk folder cerita (subfolder ber-content.json)
...
Menjalankan:
    cd direktori ini (tempat server.py berada)
    python3 server.py
lalu buka http://127.0.0.1:<port>/ di browser.

Endpoint:
    GET  /api/list                       -> array nama subfolder (langsung di root ini) yang punya content.json
    GET  /api/content?folder=<nama>      -> isi <nama>/content.json (raw text)
    POST /api/content?folder=<nama>      -> body JSON, overwrite content.json (indent rapi)
    GET  / | /editor.html | /editor.css | /editor.js

Keamanan:
    - Bind HANYA 127.0.0.1 (tidak pernah 0.0.0.0).
    - Param folder divalidasi ketat: tanpa '/', '\\', '..', dan HARUS ada di
      hasil scan folder (subfolder langsung di root ini) yang ber-content.json
      (whitelist dinamis per-request).
...
# Direktori ini = root folder editor (lokasi file server.py), supaya path benar dari mana pun dijalankan.
...
class Handler(BaseHTTPRequestHandler):
    server_version = "CeritaEditor/1.0"
        ...
        if route == "/api/content":
            ...
            if path is None:
                if is_safe_folder_name(folder):
                    self._send_json(404, {"error": "Folder tidak dikenal: '%s' (tidak ada content.json di subfolder langsung di root ini)." % folder})
                else:
...
def main():
    ...
    print("Editor cerita berjalan di: http://127.0.0.1:%d/" % port)
